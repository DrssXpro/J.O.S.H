...
const DataMapAndShow = memo(
	({ chartIndex, chartFilter, chartRequestConfig, chartConfig, chartOptions, update }: DataConfigProps) => {
		...
		const isCharts = useMemo(() => chartConfig.chartFrame === ChartFrameEnum.ECHARTS, []);

		const isNotData = useMemo(() => {
			return chartConfig.chartFrame === ChartFrameEnum.STATIC || typeof chartOptions.dataset === "undefined";
		}, []);

		// 针对 dataset 图表显示映射
		const dimensionsAndSource = useMemo<IDataType[]>(() => {
			if (!chartOptions.dataset) return [];
			const dimensions = chartOptions.dataset.dimensions;
		...
		useEffect(() => {
			...
			setCode(chartOptions.dataset ? chartOptions.dataset : "此组件无数据源");
		}, [chartOptions.dataset]);
									...
									{chartFilter ? (
										<>
											...
											<div className="flex flex-row-reverse gap-2">
												<Button
													onClick={() => {
														update(chartIndex, "filter", null, undefined);
													}}
												>
													删除
												</Button>
											...
											<div className="flex items-center justify-center">
												...
												<Button
													...
													onClick={() => {
														...
														update(chartIndex, "filter", null, filterCode);
								...
								<Modal
									...
									footer={
										<div className="flex flex-row-reverse gap-2">
											<Button
												onClick={() => {
													update(chartIndex, "option", "dataset", safeJSONParse(editCode));
										...
										<JCodeMirror
											code={
												typeof editCode === "string"
													? editCode
													: JSON.stringify(editCode, null, 2)
											}
											lan="json"
											height={600}
											changeCode={(code) => {
